...
any_missing_required() {
  ...
  file_ok "models/text_encoders/t5/t5xxl_fp16.safetensors" 9000 || missing=1
  file_ok "models/vae/FLUX1/ae.safetensors" 250 || missing=1
...
if any_missing_required; then
  ensure_hf_auth
else
  log "All required model files already appear to be present"
fi
...
download_file() {
  ...
  if [[ -f "$final_path" ]]; then
    echo "REMOVE  incomplete/small file: $final_path"
    rm -f "$final_path"
  fi
...
check_file "T5XXL FP16 text encoder" "models/text_encoders/t5/t5xxl_fp16.safetensors" 9000
check_file "Flux VAE ae.safetensors" "models/vae/FLUX1/ae.safetensors" 250
